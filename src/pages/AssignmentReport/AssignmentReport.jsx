...
export default function AssignmentReport(props) {
  ...
  async function getReport() {
    ...
    const result = await fetch(
      `${process.env.REACT_APP_URL}/moss/report/?courseWorkId=${id}`,
      {
        method: "get",
        headers: {
  ...
  function getNodes(report) {
    ...
    var nodes = [];
    report.map((matches) => {
      if (dict[matches.file1.fullName] == undefined) {
        dict[matches.file1.fullName] = {
          fullName: matches.file1.fullName,
          edgeCount: 1,
        };
      } else {
        dict[matches.file1.fullName].edgeCount++;
      }
      if (dict[matches.file2.fullName] == undefined) {
        dict[matches.file2.fullName] = {
          fullName: matches.file2.fullName,
          edgeCount: 1,
        };
      } else {
        dict[matches.file2.fullName].edgeCount++;
      }
    });
    for (id in dict) {
      nodes.push({
  ...
  function getEdges(report) {
    return report.map((matches) => {
      ...
      return {
        from: matches.file1.fullName,
        to: matches.file2.fullName,
        width: percent / 10,
              ...
              <tbody>
                {report.map((matches, index) => {
                  return (
                    <tr>
                      <td>{index + 1}</td>
                      <td title={matches.file1.fullName}>
                        {matches.file1.fullName}
                      </td>
                      <td title={matches.file2.fullName}>
                        {matches.file2.fullName}
                      </td>
                      <td>
                        {(matches.file1.percentage + matches.file2.percentage) /
                          2}
                      </td>
