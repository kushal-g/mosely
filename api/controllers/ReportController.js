...
module.exports.incrementalSync = async (req, res, next) => {
  try {
    ...
    for (user of users) {
      ...
      for (const course of courses) {
        let courseWorks = await classRoomDb.getCourseWork(tokens, course.id);
        courseWorks = courseWorks.filter((work) =>
          getLanguage(work.description)
        ); //Only sync coursework where i can find the language
        ...
        for (const work of courseWorks) {
          ...
          for (const singleSubmission of submissions) {
            ...
            const {
              name: { fullName },
              emailAddress,
            } = await classRoomDb.getUserDetails(
              tokens,
              singleSubmission.userId
            );

            client.addRawFile(
              submittedCode,
              createName(fullName, emailAddress)
            );
          }
