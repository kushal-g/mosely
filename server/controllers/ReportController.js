...
const { getLanguage, verifyFileExtension } = require('../utils/languageValidators');
		...
		for (const course of courses) {
			...
			for (const work of courseWorks) {
				...
				for (const singleSubmission of submissions) {
					...
					console.log(singleSubmission.assignmentSubmission)
					const driveId = extractDriveId(singleSubmission);
					if(!driveId){
						continue
					}
					
					const fileName = await googleDrive.getFilename(tokens,driveId)
					if(!verifyFileExtension(fileName,language)){
						continue
					}
					...
					client.addRawFile(submittedCode, createName(fileName, emailAddress));
				}
